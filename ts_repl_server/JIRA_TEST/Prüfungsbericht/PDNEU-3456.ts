...
await executeStep(
    ...
    resultWriter,
    async () => {
        const element = await bericht_ubermittlung.isVisible('links', 'UEBERMITTLUNGSSCHREIBEN');
        await executeAssertion(
            () => { expect(element).toBeTruthy() },
            `Element Übermittlungsschreiben ist NICHT sichtbar`
        );
    ...
    'Tab Übermittlungsschreiben ist sichtbar (/)',
    'Tab Übermittlungsschreiben ist NICHT sichtbar (x)',
    errors,
    goblaStatus,
    stepStatus
);
...
await executeStep(
    ...
    async () => {
        ...
        // --- Teil 2: Sichtbarkeitsprüfung ---
        const isVisible = await bericht_ubermittlung.isVisible(comboGroup, zusatzKey);
        await executeAssertion(
            () => { expect(isVisible).toBeTruthy() },
...
await executeStep(
    ...
    async () => {
        ...
        // Prüfung: Ist das Textfeld nun sichtbar und editierbar?
        const isVisible = await bericht_ubermittlung.isVisible(fieldGroup, sonstigerZusatzField);
    ...
    'Feld "Sonstiger Namenszusatz*" (Textfeld) erscheint nur bei entsprechender Auswahl und akzeptiert maximal 255 Zeichen (/)',
    'Feld "Sonstiger Namenszusatz*" ist falsch sichtbar oder die Zeichenbegrenzung wird nicht eingehalten (x)',
    errors,
    goblaStatus,
    stepStatus
);
